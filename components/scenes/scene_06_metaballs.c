...
/* Half-res render with 2x2 tile blit (same trick as scene_05_plasma).
 * Otherwise the per-pixel inverse-distance sum blows the budget. */
#define MB_W (FB_W / 2)
#define MB_H (FB_H / 2)
...
static void render(void *ctx, fb_t *fb, uint32_t t)
{
    ...
    /* 4 blobs moving in interfering circles. */
    ...
    for (int i = 0; i < N_BLOBS; ++i) {
        ...
        br[i] = (MB_W * 0.18f) + 4.0f * lut_sin(ts * 1.4f + i);   /* breathing radius */
    }

    /* Palette walk: warm orange/red early, deep purple/blue late. */
    ...
    for (int py = 0; py < MB_H; ++py) {
        ...
        for (int px = 0; px < MB_W; ++px) {
            ...
            for (int i = 0; i < N_BLOBS; ++i) {
                ...
                /* Inverse-square contribution scaled by radius^2. */
                sum += (br[i] * br[i]) / d2;
            }
            /* sum > 1.0 = inside-the-blob isosurface. */
            uint8_t intensity;
            if (sum > 1.6f) {
                intensity = 255;
            } else if (sum > 0.8f) {
                /* Ramp from 60..255 as sum goes 0.8..1.6 */
